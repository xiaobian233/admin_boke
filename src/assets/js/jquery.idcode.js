...
(function($) {
  ...
  $.idcode = {
    ...
    validateCode: function(option) {
      ...
      if (
        inputV.toUpperCase() == _storeData(_set.storeLable, null).toUpperCase()
      ) {
        //修改的不区分大小写
        return true;
      } else {
  ...
  function _setCodeStyle(eid, codeType, codeLength) {
    ...
    if (!settings.inputID) {
      htmlCode =
        '<span><input id="ehong-code-input" type="text" maxlength="4" /></span>';
    }
    htmlCode += '<div id="ehong-code" class="ehong-idcode-val ehong-idcode-val';
    htmlCode += String(randNum);
    htmlCode +=
      '" href="#" onblur="return false" onfocus="return false" oncontextmenu="return false" >' +
      _setStyle(codeObj) +
      "</div>" +
      '<span id="ehong-code-tip-ck" class="ehong-code-val-tip" >' +
      settings.codeTip +
      "</span>";
    $(eid).html(htmlCode);
  ...
  function _setStyle(codeObj) {
    ...
    for (var i = 0; i < codeObj.length; i++) {
      ...
      fnCodeObj.push(
        '<font color="' + col[charIndex] + '">' + codeObj.charAt(i) + "</font>"
      );
    }
...
export let domButton = function(dom) {
  //初始化数据
  $.idcode.setCode();
  $("#btns").click(function() {
